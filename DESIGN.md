...
- **Molecular Pattern:** A repeatable SVG pattern of nodes and lines should be used as a "watermark" in the corner of sections or as a background for hero banners, set at 5% opacity. In the hero, this pattern is a live canvas node network that drifts on its own and reacts to pointer/touch proximity (nodes and connecting lines near the cursor or finger brighten toward Lab Teal). It pauses off-screen and off-tab, and steps aside for the static watermark under reduced motion or if canvas is unavailable.