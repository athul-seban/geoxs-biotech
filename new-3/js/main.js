(function () {
  ...
  // Spatial tilt: every glass panel reads pointer position and tilts on its
  // own X/Y axis, as if it were a physical pane floating at its own depth —
  // this variant's one signature motion, layered above the shared specular glow.
  try {
    var tiltSelector = '.pillar-featured, .stat-callout, .contact-form, ' +
      '.team-grid, .pillar-grid-quad, .pillar-grid-duo, .pillar-stack';
    var reduceMotionForTilt = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    if (!reduceMotionForTilt && window.matchMedia && window.matchMedia('(hover: hover)').matches) {
      document.querySelectorAll(tiltSelector).forEach(function (panel) {
        panel.addEventListener('pointermove', function (e) {
          var rect = panel.getBoundingClientRect();
          var px = (e.clientX - rect.left) / rect.width - 0.5;
          var py = (e.clientY - rect.top) / rect.height - 0.5;
          panel.style.setProperty('--tiltx', (py * -6) + 'deg');
  ...
  // scroll, staging the hero as depth planes rather than one flat layer.
  try {
    var parallaxHero = document.querySelector('.hero');
    var parallaxGlobe = document.querySelector('.hero-visual');
    var parallaxCopy = document.querySelector('.hero-copy');
    var reduceMotionForParallax = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    if (parallaxHero && !reduceMotionForParallax) {
      ...
      window.addEventListener('scroll', function () {
        ...
        requestAnimationFrame(function () {
          ...
          if (rect.bottom > 0 && rect.top < window.innerHeight) {
            ...
            if (parallaxGlobe) parallaxGlobe.style.transform = 'translateY(' + (progress * 0.18) + 'px)';
            if (parallaxCopy) parallaxCopy.style.transform = 'translateY(' + (progress * 0.08) + 'px)';
          }
    ...
    // Hero stays static — still fully readable without parallax.
  }
